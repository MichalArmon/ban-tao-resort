 ...
 * מציג את כל ימי החודש הנוכחי (1 → סוף חודש).
 * ימים בלי סשן: disabled (אפור).
 * תומך בשני מקורות: `sessions` (אמיתי ממונגו) או `guestSchedule` (occurrences).
 */
export default function WorkshopDatePickerInline({
  sessions, // ← עדיפות ראשונה (Mongo real sessions)
  guestSchedule = [], // ← גיבוי (occurrences)
  sessionDate,
  onSelectDate,
}) {
  ...
  /** מיפוי תאריך ← רשימת סשנים, עם צבע קטגוריה אם קיים */
    ...
    <Box sx={{ mt: 2 }}>
      ...
      <Stack direction="row" flexWrap="wrap" gap={1}>
        {daysOfMonth.map((iso) => {
          ...
          const isActive = iso === sessionDate;
          ...
          return (
            <Button
              ...
              onClick={() => hasSessions && handleDateSelect(iso)}
              disabled={!hasSessions}
              sx={{
                minWidth: 44,
                minHeight: 44,
                bgcolor: !hasSessions
                  ? "rgba(0,0,0,0.08)" // אפור מלא לדיסאבל
                  : isActive
                  ? color
                  : "transparent",
